...
export default function ReviewStep({ onEditStep }: ReviewStepProps) {
  ...
  const isStripe = selectedPaymentMethodCode === 'stripe';
  const isRazorpay = selectedPaymentMethodCode === 'razorpay';
  const isCashfree = selectedPaymentMethodCode === 'cashfree';
  ...
  const handlePlaceOrder = async () => {
    ...
    setLoading(true);
    try {
      const orderCode = await placeOrderAction(selectedPaymentMethodCode);
  ...
  return (
    <div className="space-y-6">
      ...
      {isStripe && canPlaceOrder ? (
        <StripePaymentForm />
      ) : isRazorpay && canPlaceOrder ? (
        <RazorpayPaymentForm />
      ) : isCashfree && canPlaceOrder ? (
        <CashfreePaymentForm />
      ) : (
        <Button
