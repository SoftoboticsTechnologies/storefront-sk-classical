...
export default function ShippingAddressStep({ onComplete }: ShippingAddressStepProps) {
  ...
  const { addresses, countries, order, isGuest, refreshOrder, refreshAddresses } = useCheckout();

  const orderAddressMatchId = useMemo(() => {
    const shippingAddress = order.shippingAddress;
    if (!shippingAddress?.streetLine1) return null;
    const matchingAddress = addresses.find(
      (a) =>
        normalize(a.streetLine1) === normalize(shippingAddress.streetLine1) &&
        normalize(a.postalCode) === normalize(shippingAddress.postalCode) &&
        normalize(a.city) === normalize(shippingAddress.city)
    );
    return matchingAddress?.id ?? null;
  }, [order.shippingAddress, addresses]);

  const [selectedAddressId, setSelectedAddressId] = useState<string | null>(() => {
    if (orderAddressMatchId) return orderAddressMatchId;
    const defaultAddress = addresses.find((a) => a.defaultShippingAddress);
    return defaultAddress?.id || addresses[0]?.id || null;
  });

  // Re-sync whenever the order's actual shipping address (or the address book) changes,
  // so re-opening this step via "Edit" always reflects what's really set on the order.
  useEffect(() => {
    if (orderAddressMatchId) {
      setSelectedAddressId(orderAddressMatchId);
    }
  }, [orderAddressMatchId]);
