 ...
 */
export async function NavigationLink({href, prefetch = false, ...rest}: NavigationLinkProps) {
    const locale = await getRouteLocale();
    const localizedHref = href === '/' ? `/${locale}` : `/${locale}${href}`;
    // Defaults prefetch off — see @/platform/i18n/navigation's Link for why.
    return <NextLink href={localizedHref} prefetch={prefetch} {...rest} />;
}
